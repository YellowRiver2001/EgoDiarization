...
#测试3D-speaker方法
def voice2id_pyannote2_final_3D_speaker_study11(save_path,a):
    # 视频文件路径
    audio_path = save_path+'audio.wav' # 指定的音频文件路径

    # segments_path = save_path+'pyannote_powerset_Ego4d_segments.txt'   #只用segmentation模型的结果
    # segments_path = save_path+'subsegment_3D-speaker.txt'    #先用fsmn_vad进行语音活动检测，再按照1.5s进行分段
    segments_path = save_path+'segment_3D-speaker.txt'        #3d-speaker最终结果得到的segments
    # segments_path = save_path+'clean_3D_speaker_segments.txt'
    ...
    split_path = save_path+'split_3D_speaker_wav/'   #用于存放被分段的音频文件
    clean_path = save_path+'clean_3D_speaker_wav/'   #用于存放被音频增强（去噪）的音频文件
    ...
    # clean_segments = Fsmn_VAD(clean_audio_path,save_path+'clean_3D_speaker_segments.txt',model)
    clean_segments = np.loadtxt(save_path+'clean_3D_speaker_segments.txt')
    # exit(0)
    ...
    np.savetxt(save_path+'final_segments_3D_final_study10_segments.txt',segments_final,fmt='%d')
    #np.savetxt(save_path+'segments.txt',segments,)
    np.savetxt(save_path+'id_segments_3D_final_study10.txt',id_final,fmt='%d')
...
#测试3D-speaker方法
def voice2id_pyannote2_final_3D_speaker(save_path,a):
    # 视频文件路径
    audio_path = save_path+'audio.wav' # 指定的音频文件路径
