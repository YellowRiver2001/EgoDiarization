...
parser.add_argument('--videoFolder',           type=str, default="/home/rx/audio-visual/Light-ASD/demo",  help='Path for inputs, tmps and outputs')
parser.add_argument('--videos_Path',           type=str, default="/home/rx/audio-visual//v2/clips",  help='Path for inputs, tmps and outputs')
parser.add_argument('--pretrainModel',         type=str, default="weight/pretrain_AVA_CVPR.model",   help='Path for the pretrained model')

#parser.add_argument('--pretrainModel',         type=str, default="Ego4d_exps/exp5/model/model_0034.model",   help='Path for the pretrained model')
...
if args.evalCol == True:
	...
	#args.videoPath = os.path.join(args.videos_Path, args.videoName + '.mp4')
	args.duration = 0
...
def visualization(tracks, scores,fps,id_dict,n,args):   #n表示一共全局追踪到n个人

	# # 添加字幕到视频
	# font_size = 30
	# font = ImageFont.truetype("STSong.TTF", font_size)  # 替换为实际的字体文件路径
	...
	m = len(flist)
	label = np.zeros((n, m), dtype=float)	#存储每个人每帧的说话分数矩阵

	for fidx, fname in tqdm.tqdm(enumerate(flist), total = len(flist)):
		
		image = cv2.imread(fname)
		flag=0       #表明当前帧是否有说话人
		max_score=0  #找到当前最大概率分数的说话人

		
		for face in faces[fidx]:
			label[id_dict[int(face['track'])]][fidx] = face['score']
			clr = colorDict[int((face['score'] >= 0))]
			if clr==255:
				flag=1
				if face['score']>max_score:
					max_score = face['score']
					track_id = id_dict[int(face['track'])]
			txt = round(face['score'], 1)
			cv2.rectangle(image, (int(face['x']-face['s']), int(face['y']-face['s'])), (int(face['x']+face['s']), int(face['y']+face['s'])),(255,0,255-clr),int(clr/51+1))
			#cv2.putText(image,'%s'%(txt), (int(face['x']-face['s']), int(face['y']-face['s'])), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,clr,255-clr),2)
			cv2.putText(image,str(id_dict[int(face['track'])]), (int(face['x']-face['s']), int(face['y']-face['s'])), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,255,0),int(clr/51+1))
			box.append([fidx,face['track'],face['x']-face['s'],face['y']-face['s'],2*face['s'],2*face['s']])

	 
		# # 检查当前帧是否在字幕时间范围内，并添加字幕文本
		# for subtitle in subtitles:
		# 	start_time = subtitle["start"]
		# 	end_time = subtitle["end"]
		# 	if start_time <= fidx / fps <= end_time:  #如果视频帧在某个字幕时间段里
		# 		# 将帧转换为PIL图像
		# 		image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
		# 		draw = ImageDraw.Draw(image)
		# 		if flag: #说明当前人脸正在说话
		# 			if subtitle["text"][0]==' ':#代表文本已经加入了说话人序号
		# 				text = subtitle["text"]
		# 			else:
		# 				text = ' '+str(track_id)+':'+subtitle["text"]
		# 				subtitle["text"] = text
		# 		else :
		# 			text = subtitle["text"]  #若当前没检测出说话人但是又语音翻译，则默认为摄像头佩戴者,或者视频外的人
		# 		#print(text)
		# 		text_width, text_height = draw.textsize(text, font=font)
		# 		text_position = ((fw - text_width) // 2, fh - text_height - 10)
		# 		# 计算灰色条框的位置，给字幕增加一些间距
		# 		box_position = [(text_position[0] - 10, text_position[1] - 10), (text_position[0] + text_width + 10, text_position[1] + text_height + 10)]
		# 		# 添加灰色条框
		# 		draw.rectangle(box_position, fill ="grey")
		# 		draw.text(text_position, text, font=font, fill=(255, 255, 255))
		# 		# 将PIL图像转换回OpenCV图像
		# 		image = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
		vOut.write(image)
	#np.savetxt(args.pyaviPath+'/short_id.txt', box, fmt='%d') 
	np.savetxt(args.pyaviPath+'/track_id.txt', box, fmt='%d') 
	np.savetxt(args.pyaviPath+'/speaker_global_Ego4d.txt', label) 
...
def make_id_dic(short_path,global_path):
	short_id = []
	global_id = []
	id_dict = {}
	...
	with open(global_path, 'r') as file:
		...
		for line in lines:
			...
			row = line.strip().split(' ')
			# 获取每行short_id
			global_id.append(row[1])
	for i in range(len(short_id)):
		id_dict[int(short_id[i])]=int(global_id[i])
...
def main():
	...
	args.videoPath = glob.glob(os.path.join(args.videos_Path, args.videoName + '.*'))[0]
	args.savePath = os.path.join(args.videoFolder, args.videoName)
	...
	audio_path = args.audioFilePath  # 指定的音频文件路径
	...
	#生成短期追踪id文件
	short_track(vidTracks, scores)
	#进行长期追踪
	os.system('cd global_tracking; python3 make_trajectories.py ' +args.pyaviPath +'; python3 group_fast.py; python save_results_ascii.py ' + args.pyaviPath) 
	
	#生成短期追踪id和长期追踪的对应字典
	filename = 'global_tracking/short2global.txt'
	os.system('cp global_tracking/short2global.txt '+args.pyaviPath+'/short2global.txt')
	...
	if args.evalCol == True:
		...
		visualization(vidTracks, scores,fps,id_dict,n,args)
