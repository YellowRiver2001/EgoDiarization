...
if __name__ == '__main__':
    ...
    x = torch.zeros(10, 10, 80)
    model = CAMPPlus(feat_dim=80, embed_dim=192, pooling_func='TSTP')
    #model.load_state_dict(torch.load("/Users/twc/Desktop/cam/camplus.ckpt", map_location="cpu"))
    y = model(x)
    print(y.shape)
    '''
    model.eval()
    ...
    torch.onnx.export(
        model, dummy_input, "camplus.onnx", do_constant_folding=True,
        verbose=False, opset_version=14, input_names=['feats'], output_names=['embs'],
