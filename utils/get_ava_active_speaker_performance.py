...
def calculate_precision_recall(df_merged):
  ...
  np.savetxt('./Ego4d_exps/exp5/precision.txt',np.array(df_merged["precision"]))
  #indices = np.where(df_merged["precision"] == 0.8)[0]  # 返回满足条件的索引数组

  #np.savetxt('./Ego4d_exps/exp5/precision_threshold.txt',[df_merged["score"][indices]])


  df_merged["recall"] = df_merged["tp"] / all_positives
  #np.savetxt('./Ego4d_exps/exp5/recall.txt',np.array(df_merged["recall"]))
  # # 使用 matplotlib 绘制曲线图
  # plt.figure()  # 设置图形的大小
  # plt.plot(np.array(df_merged["recall"]), np.array(df_merged["precision"]), marker=None, linestyle='-', color='b', label='曲线图')  # 绘制曲线图，设置标记点、线型、颜色和图例
  # plt.xlabel('Recall')  # 设置 x 轴标签
  # plt.ylabel('Precision')  # 设置 y 轴标签
  # plt.title('P-R curves')  # 设置图形标题
  # plt.legend()  # 显示图例
  #plt.grid(True)  # 添加网格线

  # 保存图形到文件
  # plt.savefig('./Ego4d_exps/exp5/P-R.png')  # 保存为 PNG 格式的文件，文件名为 curve_plot.png
  # np.savetxt('./Ego4d_exps/exp5/sub_score.txt',np.array(df_merged["score"]))
...
#注：这里有修改，将返回值mAP，改成三个mAP,precision, recall
def run_evaluation(groundtruth, predictions):
  ...
  print("average precision: %2.2f%% precision: %2.2f%% recall: %2.2f%%"%(mAP,np.mean(precision)*100, np.mean(recall)*100))
...
def main():
  ...
  del args.verbose
  mAP,precision,recall = run_evaluation(**vars(args))
